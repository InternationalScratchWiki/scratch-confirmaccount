...
require_once __DIR__ . '/../backend/ScratchWiki.php';
...
class AccountRequestSubmission {
	...
	public function submit( IContextSource $context ) {
		...
		if (in_array(strtolower($u->getName()), $acc_req_blocked_usernames)) {
			return [
				'accountreq_blocked',
				$context->msg( 'requestaccount-username-blocked' )->text()
			];
		}
		...
		switch (vercode_recently_commented_by_user($context->msg('requestaccount-project-link')->text(), $this->userName)) {
			case SCRATCH_API_FAILURE:
				return array('api_failed', $context->msg('requestaccount-api-failed'));
			case SCRATCH_COMMENT_NOTFOUND:
				return array('no_comment', $context->msg('requestaccount-nocomment-error'));
			case SCRATCH_COMMENT_FOUND:
				//do nothing and continue if the code was commented
				break;
			default:
				assert(false);
		}
		...
		$user_res = $dbr->select(
			...
			array('1'),
			array(
				'LOWER(user_name)' => strtolower($u->getName())
			),
			__METHOD__,
			array()
		);
		
		if ( 0 != $u->idForName() || AuthManager::singleton()->userExists( $u->getName() ) || $user_res->numRows() > 0) {
			return [
