...
class RequestAccountPage extends SpecialPage {
	...
	protected function showForm( $msg = '', $forgotFile = 0 ) {
		...
		$vercode = scratchwiki_verification_code();
		...
		if ( count( $wgAccountRequestTypes ) > 1 ) {
			...
			$options = [];
			# Kenny2scratch on 2020-04-05: disabled default
			$options[] = Xml::option(
				$this->msg( 'requestaccount-level-select' )->escaped(), null, !$this->mType, ['disabled']
			);
		...
		if ( $this->hasItem( 'CV' ) || $this->hasItem( 'Notes' ) || $this->hasItem( 'Links' ) ) {
			...
			if ( $this->hasItem( 'Notes' ) ) {
				...
				style='width: 100%; background-color: #f9f9f9;' minlength='100' required>" .
	...
	protected function doSubmit() {
		...
		# modified by Kenny2scratch, 2020-04-12
		if ( count($wgAccountRequestTypes) > 1 && !$this->mType ) {
			$this->showForm( $this->msg( 'nolang' )->escaped() );
			return;
		}
